...
def verificar_colisoes():
    ...
    for ax, ay in asteroides:
        ...
        if distancia < (RAIO_NAVE + RAIO_ASTEROIDE):
            print("💥 COLISÃO COM A NAVE!")
            x, y = 0.0, 0.0
            velocidade = 0.0
            break

def verificar_colisoes_misseis():
    global misseis, asteroides, asteroide_angulos, asteroide_formas, asteroide_velocidades

    novos_misseis = []
    asteroides_remover = set()
    ...
    for m in misseis:
        ...
        for i, (ax, ay) in enumerate(asteroides):
            distancia = math.sqrt((mx - ax) ** 2 + (my - ay) ** 2)
            if distancia < (RAIO_ASTEROIDE + RAIO_MISSIL):
                asteroides_remover.add(i)
                colidiu = True
                break

        if not colidiu:
            novos_misseis.append(m)

    misseis = novos_misseis

    for i in sorted(asteroides_remover, reverse=True):
        del asteroides[i]
        del asteroide_angulos[i]
        del asteroide_formas[i]
        del asteroide_velocidades[i]
